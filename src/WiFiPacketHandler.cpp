...
void WiFiPacketHandler(void* buff, wifi_promiscuous_pkt_type_t type) {
    ...
    static uint8_t DoubleMac[12];                                                       //With cheese
    static char msg[80];
    static uint8_t bcast[6]={255,255,255,255,255,255};                                  //Broadcast mac
    static uint8_t BeaconStore[BEACON_CACHE_SIZE][12];                                  //12 bytes for two macs
    extern QueueHandle_t PacketQueue;                                                   //Read by DeviceAdd routine                  
